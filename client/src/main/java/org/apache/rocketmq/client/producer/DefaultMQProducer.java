 ...
 */
/**
 * 这是应用程序使用rocketmq发送消息的入口类
 * 先实例化DefaultMQProducer一个实例，然后调用send方法发送消息
 * DefaultMQProducer负责配置mq的各种参数，并转发消息，消息的具体发送过程由DefaultMQProducerImpl实现
 */
public class DefaultMQProducer extends ClientConfig implements MQProducer {
    ...
    /**
     * 异步传输数据的接口
     * 在一个集群中可以配置一台机器专门负责消息轨迹的收集工作
     *  该台机器上配置traceTopicEnable = true
    ...
    public DefaultMQProducer(final String producerGroup, RPCHook rpcHook, boolean enableMsgTrace,
        ...
        //enableMsgTrace为true，表示开启消息轨迹跟踪
        if (enableMsgTrace) {
            try {
                //创建一个轨迹消息发送者，一个线程池用于异步执行消息发送
    ...
    public void start() throws MQClientException {
        //将namespace添加到producerGroup中
        this.setProducerGroup(withNamespace(this.producerGroup));
        this.defaultMQProducerImpl.start();
        if (null != traceDispatcher) {
            try {
                //启动消息追踪的线程，循环发送100条消息
